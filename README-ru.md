...
Предыстория
--
Длительное время писал код на JS и при этом собирал часто используемы фрагменты кода в одном файле чтобы было легче копипастить. В итоге этот набор разросся до того что его целесообразней было оформить как библиотеку.
...
Почему CoffeeScript
--
Быблиотека могла быть написана на оригинальном JavaScript, однако на CoffeeScript писать проще, короче и быстрее. В общем, незачем создавать себе сложности.
...
`$F.prototype.default`
--
Подставляет значение как результат работы алгоритма при возникновении исключения.
...
`$F.prototype.cell`
--
Реализует API подобное клеткам в Clojure. В качестве параметров указываются другие клетки, значения которых применяются к текущей функции.
...
`$F.as`
--
Проверяет является ли второй аргумент типом первого. В качестве типа можно задавать как конструкторы, так и строковые аналоги(используется typeof).

`$F.cell`
--
Реализует API подобное клеткам в Clojure. Создаёт клетку значение которой можно менять.

Примечания: В данную библиотеку слабо вписывается, возможно будет вынесено отдельно в качестве дополнения.

`$F.inherit`
--
Возвращает декаоратор с поведением как у `$F`. Посути это его потомок.
